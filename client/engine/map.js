...
var Map = function(options){
    ...
    this.regionsModels = new client.models.RegionCollection();
...
Map.prototype.getRegionByTileXY = function(tileX, tileY){
    var regionXY = ZT.shared.mapHelper.regionXYByTileXY(tileX,tileY);
    return this.regionsModels.get(client.models.RegionModel.generateId(regionXY.x,regionXY.y),"pos");
};

Map.prototype.getTileByTileXY = function(tileX, tileY){
    return this.tiles[tileX + ":" + tileY];
};

Map.prototype.getTileByWorldXY = function(worldX, worldY){
...
Map.prototype.getTileSlotByWorldXY = function(worldX, worldY){
...
Map.prototype.addTile = function(tileModel){

    var x = tileModel.get('x');
    var y = tileModel.get('y');

    var tile = new client.engine.Tile({
        game: this.game,
        model: tileModel,
        x: x,
        y: y,
 ...
 */
Map.prototype.centerTo = function(toX,toY){
    ...
    for(var x = this.firstRegionX; x <= this.lastRegionX; x++){
        for(var y = this.firstRegionY; y <= this.lastRegionY; y++){
            ...
            this.cachedRegions[client.models.RegionModel.generateId(x,y)] = true;

            //check if already fetched region
            if(this.regionsModels.get(client.models.RegionModel.generateId(x,y),"pos")){
                continue;
            }
 ...
 * Manage cache tiles
 */
Map.prototype.updateCachedTiles = function(sync){

    var toRemove = [];

    this.regionsModels.each(function(regionModel){
        if(this.cachedRegions[regionModel.pos] && !this.lastCachedRegions[regionModel.pos]){
            regionModel.tiles.each(function(tileModel){
                this.addTile(tileModel);
            },this);
        }else if(this.lastCachedRegions[regionModel.pos] && !this.cachedRegions[regionModel.pos]){
 ...
 */
Map.prototype.updateVisibleTiles = function(sync){
...
Map.prototype.moveRelative = function(relX, relY){

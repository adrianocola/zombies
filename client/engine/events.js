...
Events.prototype.handleEvents = function(){

    var that = this;

    socket.on(events.PLAYER_MOVE,function(event){
        Logger.debug("Handling event " + events.PLAYER_MOVE + ": " + event.id);
        ...
        if(!fromRegionModel){
            Logger.debug(events.PLAYER_MOVE + ": player moved in from uncached region");
            var model = new PlayerModel(event.data.player);
            var player = new client.engine.Thing({tile: that.game.map.getTileByTileXY(model.get("x"),model.get("y")), slot: model.slot, model: model, game: that.game, image:"walking", animation: 'walk'});
        ...
        //if moved out from cached region, remove player
            ...
            Logger.debug(events.PLAYER_MOVE + ": player moved from cached region");
        ...
        //if have from and to, just move player
            ...
            Logger.debug(events.PLAYER_MOVE + ": player moved between cached regions");
            var player = that.game.players[event.data.player._id];
    ...
    socket.on(events.PLAYER_JOIN,function(event){
        if(that.events[event.id]) return;
        if(that.game.playerModel.id === event.actor) return;
